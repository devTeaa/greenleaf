...
type Config = {
  ...
  // Shared mode: ports for Nuxt + backend (host-bound)
  nuxtPort: number
  backendPort: number
  nuxtBind: string
  backendBind: string
...
async function main() {
    ...
    options: [
      {
        ...
        hint: 'you provide the reverse proxy; we expose Nuxt + backend on localhost ports',
      },
      {
        value: 'standalone',
  ...
  // ----- 2. Ports -----
  let nuxtPort = 3051
  let backendPort = 3052
  let nuxtBind = '127.0.0.1'
  let backendBind = '127.0.0.1'
  ...
  if (mode === 'shared') {
    ...
    const cp = await p.text({
      message: 'Pocketbase (backend) port on host',
      initialValue: String(existing?.backendPort ?? 3052),
      validate: (v) => (!/^\d+$/.test(v ?? '') ? 'Must be a number' : undefined),
    })
    if (p.isCancel(cp)) return bail()
    backendPort = Number(cp)
  ...
  const backendSubdomain = await p.text({
    message: 'Backend / admin subdomain',
    initialValue: existing?.backendSubdomain ?? `backend.${domainValue}`,
    validate: (v) => (!v?.trim() ? 'Required' : !isValidDomain(v.trim()) ? 'Invalid' : undefined),
  })
  if (p.isCancel(backendSubdomain)) return bail()
  ...
  p.note(
    mode === 'shared'
      ? `Credentials for the Pocketbase admin dashboard at https://${(backendSubdomain as string).trim()}/_/`
  ...
  if (useGeneratedPassword) {
    ...
    p.log.warn(DIM + 'Save this — you will need it to log in to the backend admin.' + RESET)
  } else {
  ...
  const config: Config = {
    ...
    backendSubdomain: (backendSubdomain as string).trim(),
    nuxtPort,
    backendPort,
    nuxtBind,
    backendBind,
  ...
  const envLines = [
    ...
    '',
    '# ----- Deployment mode -----',
    `# shared   = Nuxt + backend exposed on localhost ports (you provide reverse proxy)`,
    ...
    '# Public backend URL — the browser uses this for client-side navigation.',
    `NUXT_PUBLIC_PAYLOAD_URL=https://${(backendSubdomain as string).trim()}`,
    '',
    '# ----- Backend (Pocketbase) -----',
    ...
    '# NUXT_PUBLIC_PAYLOAD_URL=http://backend:3001',
  ]
  ...
  p.log.success(`backend/.env written (for setup + seed scripts)`)
  ...
  if (mode === 'shared') {
    p.note(
      [
        ...
        `Backend admin → ${backendBind}:${backendPort}/_/`,
        ``,
        ...
        `  server_name ${config.backendSubdomain};  → proxy_pass http://127.0.0.1:${backendPort};`,
      ].join('\n'),
